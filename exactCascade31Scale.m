    ...
    for i = 1 : m-2
        for j = i+1 : m-1
            for k = j+1 : m
                ...
                eq12 = norm(kxky(i,:))~=norm(kxky(j,:));
                eq23 = norm(kxky(j,:))~=norm(kxky(k,:));
                eq31 = norm(kxky(k,:))~=norm(kxky(i,:));
                if (abs(dwf123) < dw) && eq31 && eq23
...
save('k100Scale.mat', 'solf0', 'solf','Kx','Ky','N','kx','ky');
